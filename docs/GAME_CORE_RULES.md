...
- When another enemy remains, the base defeat transition keeps board input available instead of locking it for the defeat and entrance animations. A mechanic-owned action that is still busy may retain its own input lock. Drops already active when the enemy dies, and drops started during the transition, do not advance the next enemy's action countdown.
...
- Ordinary Godot-physics levels can select fixed mass or stage-scaled mass through the shared physics Resource. The current shared setting uses displayed stage 1 as mass `1.0` and adds `0.1` per later displayed stage, reaching mass `2.0` at displayed stage 11; the custom-physics laboratory keeps its isolated equal-mass solver. A documented gimmick may temporarily multiply a ball's resulting base mass.
...
- When an enemy has begun its defeat transition and another enemy remains in the same level, the player may keep dropping. Normal merges still grant their score and Skill Gauge once. Merge attacks that reach the delayed attack-request point before the next enemy is configured are retained, then create their normal projectiles toward that enemy once it is ready. Flushing a retained request does not emit merge registration/completion again, grant score or gauge a second time, or damage ice again.
