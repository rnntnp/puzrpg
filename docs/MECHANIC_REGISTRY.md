...
| 39 | 무게 기반 파괴 지형 | One-way floors accumulate the mass of balls above them and collapse when their configured break weight is reached. | Modular · 3 enemies · [level](../resources/levels/test_weight_break_terrain.tres) · [handler](../scripts/gimmicks/handlers/weight_break_terrain_handler.gd) | `NEEDS_PLAYTEST` | 2, 10, 25, 33, 34, 36, 37, 42 |
...
| Recovery ingestion | Campaign level 3 marks and swallows the highest-stage ball; durability can interrupt and return it to the queue | `NEEDS_PLAYTEST` | Ball capture, telegraphed targeting, durability shield, queue insertion |
