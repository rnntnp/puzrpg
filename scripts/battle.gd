...
func _load_level() -> void:
	...
	status_label.visible = level_data.show_battle_status_label
...
func update_gimmick_ui(primary: String, detail: String) -> void:
	var show_labels := level_data == null or level_data.show_gimmick_status_labels
	gimmick_action_label.visible = show_labels and not primary.is_empty()
	gimmick_detail_label.visible = show_labels and not detail.is_empty()
...
func show_first_ingestion_swallow_tutorial() -> void:
	if (
		ingestion_swallow_tutorial_shown
		or level_data == null
		or not level_data.ingestion_tutorial_enabled
		or not battle_running
		or level_finished
	):
		return
	ingestion_swallow_tutorial_shown = true
	merge_game.set_input_enabled(false)
	await get_tree().create_timer(1.0, true, false, true).timeout
	if not is_inside_tree() or not battle_running or level_finished or not right_fighter.is_alive():
		return
	right_bar.start_durability_glow()
	var tutorial := StageIntroSequenceClass.new()
	add_child(tutorial)
	tutorial.play_custom_box_spotlight_tutorial(
		"구슬을 삼켰어요!\n보호막을 뚫으면 뱉을거에요!",
		Vector2(536.0, 145.0),
		Vector2(140.0, 29.0),
		false,
